...
def collate(examples: List[Dict]) -> Dict[str, torch.Tensor]:
    ...
    for e_id, example in enumerate(examples):
        ...
        column_token_position_to_column_ids[column_token_position_to_column_ids == column_token_to_column_id_fill_val] = max_column_num
...
def load_epoch(file_prefix: Path, shard_num: int, valid_indices: Set = None):
    ...
    for shard_id in range(shard_num):
        ...
        for chunk_id in range(shard_size):
            idx += 1
            ...
            table_data = row_data_sequences[start_index: end_index]
            ...
            example['masked_context_token_positions'] = mlm_data_sequences[s1: s2]
            example['masked_context_token_label_ids'] = mlm_data_sequences[s2: s3]
            example['masked_column_token_column_ids'] = mlm_data_sequences[s3: s4]
            example['masked_column_token_label_ids'] = mlm_data_sequences[s4: s5]
